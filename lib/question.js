...
function createNext(app) {
  if (!app.options.next) return;
  if (typeof app.options.next === 'function') {
    app.next = app.options.next.bind(app);
    return;
  }
  if (typeof app.options.next === 'string') {
    app.next = function(answer, questions, cb) {
      questions.ask(app.options.next, cb);
    };
  }
}
...
Question.prototype.init = function(opts) {
  ...
  var self = this;
  this.ctx = {};
  this.ctx.inspect = function() {
    var msg = opts.message;
    if (msg[msg.length - 1] !== '?') msg += '?';
    var val = self.answer.get(self.locale) || 'nothing yet';
    return '<Question "' + msg + '" <' + val + '>>';
  };
};

Question.prototype.next = function(answer, questions, cb) {
